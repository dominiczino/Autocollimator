...
class CrosshairDetector():
    ...
    def findCrosshairCenter(self,image,crosshairKernel,smoothKernel,printTimes=False,showImgs=True):
        time1=time.time()

        #Normalize the incoming image to prevent saturation
        initial=image*0.1 + 128 
        time2=time.time()
        totalX,totalY=initial.shape
...
for frame_count, frame in enumerate(imageio.imiter("<video0>")):
    ...
    pos=detector.findCrosshairCenter(realImage,crosshairKernel,smoothKernel,showImgs=False)
    ...
    jsonboi={"x":round(sum(xVals)/N,1),"y":round(sum(yVals)/N,1)}
    try:
        socket.send(bytes(json.dumps(jsonboi),'ascii'))
        msg=None
        while msg==None:
            try:
                ...
                time.sleep(0.01)
    except Exception as boop:
        print(boop)
        print("Failed to post data to plotting server")
    
    try:
        posMod=(round(sum(xVals)/N,1),round(sum(yVals)/N,1))
        print(posMod)
    except:
